...
const UserList = () => {
    ...
    const activateUser = async (e, id) => {
        ...
        await fetch(`http://localhost:8080/api/user/updateActiveValue/${id}`, {
            method: "PUT",
            headers: {
    ...
    const deactivateUser = async (e, id) => {
        Active = e.target.value;
        await fetch(`http://localhost:8080/api/user/updateActiveValue/${id}`, {
            method: "PUT",
            headers: {
            ...
            renderCell: (params) => {
                return (
                    <div className='ms-2'>
                        ...
                        <Button variant="danger" onClick={() => handleDelete(params.row._id)}>Delete</Button>
                    </div>
                )
            }
        },
    ...
    const getUsers = async () => {
        let result = await fetch('http://localhost:8080/api/admin/get_user_data');
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    //fetch api  
                    fetch(`http://localhost:8080/api/admin/user_delete/${id}`, {
                        method: "DELETE",
                        headers: {
                            'Content-Type': 'application/json',
                            'Accept': 'application/json'
